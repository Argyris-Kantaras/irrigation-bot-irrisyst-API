...
def export_system_excel(system_name, valves, fetrikit, ec_ph, weather_station ):
    rtu2x2 = 0
    rtu4x4 = 0
    rtu2x2_radio = 0
    rtu_expandable = 0
    ...
    if system_name == 'multicable':
        parts = SYSTEM_PARTS[system_name].copy()
        for v in groups_of_valves[0]:  
          valves = valves + v
          for p in parts:
    ...
    if system_name == 'radionet':
        parts = SYSTEM_PARTS[system_name].copy()
        solar_panel_qty = 0
        for r in range(len(groups_of_valves[0])):
        ...
        print(solar_panel_qty)
            
    summary = pd.DataFrame({
        "Parameter": ["system", "fetrikit", "ec_ph", "weather_station"],
        "Value": [system_name,  "Yes" if fetrikit else "No" ,  "Yes" if ec_ph else "No" ,  "Yes" if weather_station else "No" ],
...
def create_valves_groups(valves_groups):
    ...
    global groups_of_valves
    groups_of_valves.append(final_groups)
    return f"Do you want me to add {final_groups} valves to the system? (Please answer with yes or no)"


def choose_system(field_area):
    print(field_area)
    if field_area < 40 :
        system_name="Multicable"
        return "Multicable"
    if field_area > 40 :
        system_name="Singlenet"
        return "Singlenet or Radionet"
        
def valves_qty():
    
    return "How many valve groups do you need? (Please enter accrdingly. For examaple, 3 groups of 2 valves and 1 group of 4 valves)"

    
def generate_parts(system_name, valves, fetrikit, ec_ph, weather_station):
    parts = SYSTEM_PARTS[system_name].copy()
    if system_name == "Singlenet":
        for p in parts:
            return p
...
def design_system(user_input: str = Form(...)):
    numbers = re.findall(r"\d+",user_input)
    if not numbers:
       return {"error": "Please include the required information for system design in your input."}
        
    field_area = float(numbers[0])
    system = choose_system(field_area)
    parts = generate_parts(system, groups_of_valves, fertikit, ec_ph, weather_station)
    filename = export_system_excel(system, groups_of_valves, fertikit, ec_ph, weather_station)
...
with open("D:\python projects\AI\chatbot_from_scratch\knowledge.txt", "r") as f:
...
def respond(intent,user_input):   
    ...
    if intent == "confirm":
        global fertikit, ec_ph, weather_station
        ...
        print(f"Fertikit: {fertikit}, EC/PH: {ec_ph}, Weather Station: {weather_station}")
    ...
    if intent == "export_excel":
        ...
        system_name = input("Please enter the system name for the Excel file: ")
        parts = generate_parts(system_name, groups_of_valves, fertikit, ec_ph, weather_station)
        export_system_excel(system_name, groups_of_valves,  fertikit, ec_ph, weather_station)
        return f"Excel file for {system_name} has been created successfully!"
    

    if intent == "create_valves_groups":
       valves = create_valves_groups(user_input)
       return valves
    ...
    if intent == "help":
        return "I can chat, say hi or talk about the weather."
